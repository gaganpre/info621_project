...
def build_citation_graph_v1(root_id, data, root_title=None):
    ...
    for pid in cited_papers:
        for rw in data[pid].get('related_works', []):
            rw_id = extract_id(rw)
            if rw_id not in cited_papers and rw_id != root_id:  # Avoid duplicates
                related_works.add(rw_id)
                G.add_node(rw_id, label=f"Ref {rw_id}", type='related')

    # Step 4: Add edges
    edges = []
    # Edges from cited papers to root paper
    for pid in cited_papers:
        pid_extracted = extract_id(pid)
        if pid_extracted != root_id:  # Avoid self-loop
            G.add_edge(pid_extracted, root_id)
            edges.append((pid_extracted, root_id))
    # Edges from related works to their cited papers
    for pid in cited_papers:
        pid_extracted = extract_id(pid)
        for rw in data[pid].get('related_works', []):
            rw_id = extract_id(rw)
            if rw_id in related_works:  # Only add edges for related works in the graph
                G.add_edge(rw_id, pid_extracted)
                edges.append((rw_id, pid_extracted))

    print(f"DEBUG: Generated {len(edges)} edges (cited→root: {len(cited_papers) - (1 if root_id in [extract_id(p) for p in cited_papers] else 0)}, related→cited: {sum(len(data[pid].get('related_works', [])) for pid in cited_papers if extract_id(pid) in related_works or extract_id(pid) in cited_papers)})")  # Debugging
    return G, edges



def graph_retrieval_v1(graph, query_text, top_k=3, max_hops=1):
    relevant_nodes = []
    for node in graph.nodes(data=True):
        node_id, node_data = node
        if "label" in node_data and query_text.lower() in node_data["label"].lower():
